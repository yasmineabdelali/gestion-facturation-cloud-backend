...
export class OffresService {
  ...
  async importExcel(projetId: number, file: Express.Multer.File): Promise<OffreFinanciere> {
    ...
    // Archiver l'ancienne offre active si elle existe (historique)
    ...
    // Créer les lignes de ressources associées
  ...
  private mapRowToRessource(row: any): Partial<RessourceOffre> | null {
    // Recherche flexible des noms de colonnes (insensible à la casse, variations possibles)
    ...
    if (!ressourceCloud || quantite === undefined || !unite || prixUnitaire === undefined) {
      return null; // ligne incomplète, on l'ignore
    }
...
private parseLignePdf(ligne: string): Partial<RessourceOffre> | null {
  // Format attendu : "Ressource Cloud: X | Type: Y | Quantité: Z | Unité: W | Prix unitaire: P | Description: D"
  ...
  if (!matchRessource || !matchQuantite || !matchUnite || !matchPrix) {
    return null; // ligne ne correspond pas au format attendu, on l'ignore
  }
